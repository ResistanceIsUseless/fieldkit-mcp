...
Usage:
  ...
  python recon_mcp_server.py [--port 8000] [--stdio]
"""
...
SERVER_PORT = int(os.environ.get("RECON_MCP_PORT", "8000"))
...
mcp = FastMCP("recon_mcp") if USE_STDIO else FastMCP("recon_mcp", port=SERVER_PORT)
...
@mcp.tool(
    name="recon_katana",
    annotations={
        "title": "Katana — Web Crawler & Spider",
...
@mcp.tool(
    name="recon_status",
    annotations={
        "title": "Recon MCP — Status & Dependency Check",
        "readOnlyHint": True,
        "destructiveHint": False,
        "idempotentHint": True,
        "openWorldHint": False,
...
async def recon_status(params: StatusInput) -> str:
    ...
    python_integrations = {
        "duckduckgo-search": {
            ...
            "description": "DuckDuckGo search (for recon_google_dork, default engine)",
        },
        "googlesearch-python": {
            "installed": GOOGLE_SEARCH_AVAILABLE,
            "description": "Google search execution (for recon_google_dork)",
        },
        "shodan": {
            "installed": SHODAN_AVAILABLE,
            "description": "Shodan API integration (for recon_shodan_* tools)",
        },
    }
    ...
    lines = ["## Recon MCP — Environment Status\n", "### Tool Binaries\n"]
...
if __name__ == "__main__":
    ...
    if args.stdio or os.environ.get("MCP_TRANSPORT", "").lower() == "stdio":
        logger.info("Starting recon_mcp in stdio mode")
        mcp.run(transport="stdio")
    else:
        logger.info("Starting recon_mcp on port %d (streamable HTTP)", args.port)
